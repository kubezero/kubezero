# KubeZero GCP Management Package
...
2. **Copy the package**
   - Copy the entire `gcp-management` folder into your `registry` directory (e.g., `registry/gcp-management`).
     ```shell
     cp -a packages/gcp-management registry/gcp-management
     ```
   - Commit it to your repository:
     ```shell
     git add registry/gcp-management
     git commit -m "feat: enable gcp-management package"
     ```

3. **Deploy with ArgoCD or Kustomize**
   - **ArgoCD**: Point your Application at the `registry/gcp-management` directory or use the provided `gitops.yaml`.
